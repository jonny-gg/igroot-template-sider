...
import { Card, Button, Form, Icon, Col, Row, DatePicker, TimePicker, Input, Select, Popover } from 'igroot'
...
@Form.create()
export class AdvancedForm extends PureComponent {
  state = {
    width: '100%',
  }
  componentDidMount() {
    ...
    const validate = () => {
      validateFieldsAndScroll((error, fieldsValue) => {
        if (!error) {
          const values = {
            ...fieldsValue,
            dateRange: fieldsValue.dateRange && fieldsValue.dateRange.map(item => item.format('YYYY-MM-DD')),
            dateRange2: fieldsValue.dateRange2 && fieldsValue.dateRange2.format('hh:mm:ss'),
          }
          // submit the values
          console.log(values, '最终提交了什么数据')
        }
      })
    }
